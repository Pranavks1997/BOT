...
contract BCTrustV2 {
	...
	function BCTrustV2_AddNode (uint8 _category, uint8 _grpId, uint8 _id, uint256 _r, uint256 _s) public
	{
		...
		if (address(BCTrustMember [_id]) != address(0))
			revert() ;

		if (_category == 0) {
			if (address(grpIdMasters[_grpId]) != address(0))
		        	revert() ;
		    	else {
	...
	function BCTrustV2_SaveNode(address _addr, uint8 _grpId, uint8 _id) private {
		
		ids[_addr] = BCTrustV2_Concat2Bytes(_grpId, _id) ;

	}

	function BCTrustV2_Concat2Bytes (uint8 v1, uint8 v2) private returns (bytes memory) {
